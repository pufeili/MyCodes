...
'''
PyTorch 设计了两种hook：register_forward_hook 和register_backward_hook
分别用来获取正/反向传播时，中间层模块输入和输出的feature/gradient
大大降低了获取模型内部信息流的难度
1.  register_forward_hook的作用是获取前向传播过程中，各个网络模块的输入和输出。
    对于模块module，其使用方式为：module.register_forward_hook(hook_fn) 
    其中 hook_fn的签名为：hook_fn(module, input, output) -> None    
'''
flag = True
# flag = False
if flag:
